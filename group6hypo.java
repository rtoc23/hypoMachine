...
public class group6hypo {
    ...
    // DESC : Read interrupt ID number. Based on the interrupt ID, service the interrupt
    // INPT : N/A
    // OUTP : N/A
    // RTRN : N/A
    // AUTH	: Kevin Pinto
    // *****
    public void checkAndProcessInterrupt() {
        //Prompt and read interrupt ID by first displaying possible interrupts
        ...
        System.out.println("\nEnter the Interrupt ID: ");
        int interruptID = sc.nextInt();
        System.out.println("Interrupt ID: " + interruptID);

        //Process interrupt based on interrupt ID
        switch (interruptID) {
            case 0: //No Interrupt
            ...
            case 4: //Output Operation Completion (io_putc)
                System.out.println("Output Operation Completion (io_putc)");
    ...
    // HW2 //
    // *****
    // NAME : ISRrunProgramInterrupt()
    // DESC : Read filename and call CreateProcess to run the program
    // INPT : N/A
    // OUTP : N/A
    // RTRN : N/A
    // AUTH	: Kevin Pinto
    // *****
    public void ISRrunProgramInterrupt() {
    ...
    // HW2 //
    // *****
    // NAME : ISRinputCompletionInterrupt()
    // DESC : Read PID of the process completing the io_getc operation and read one character
    //        from the keyboard and store it in the GPR of the process.
    // INPT : N/A
    // OUTP : N/A
    // RTRN : N/A
    // AUTH	: Kevin Pinto
    ...
    public void ISRinputCompletionInterrupt() {
        ...
        Scanner sc = new Scanner(System.in);
        System.out.println("Enter the PID of the process completing input completion interrupt: ");
        int PID = sc.nextInt();
        ...
        while (currentPCBptr != EOL) {
            ...
            if (RAM[currentPCBptr + PID] == PID) {
                ...
                //Read one character from the keyboard and store it in the GPR of the process
                System.out.println("Enter one character from the keyboard: ");
                char character = sc.next().charAt(0);
                RAM[currentPCBptr + GPR0] = character;

                /*//Print the character in the GPR in the PCB
                System.out.println("Character in the GPR in the PCB: " + RAM[currentPCBptr + GPR0]);*/
    ...
    // HW2 //
    // *****
    // NAME : ISRoutputCompletionInterrupt()
    // DESC : Read PID of the process completing the io_putc operation and print the character
    //        in the GPR of the process. If no match is found in WQ, then search RQ.
    // INPT : N/A
    // OUTP : N/A
    // RTRN : N/A
    // AUTH	: Kevin Pinto
    ...
    // HW2 //
    // *****
    // NAME : ISRshutdownSystem()
    // DESC : Terminate all processes in RQ and WQ and exit from the program.
    ...
    public void ISRshutdownSystem() {
        ...
        while (currentPCBptr != EOL) {
            ...
            //Set RQ to next PCB using ptr
            RQ = RAM[currentPCBptr + NextAddress];

            //Terminate the process using the current PCBptr
            terminateProcess(currentPCBptr);
        ...
        currentPCBptr = (int) WQ;

        //While currentPCBptr is not EOL
        while (currentPCBptr != EOL) {

            //Set WQ to next PCB using ptr
            WQ = RAM[currentPCBptr + NextAddress];
    ...
    // HW2 //
    // *****
    // NAME : SearchAndRemovePCBfromWQ (long pid)
    // DESC : Search the WQ for the matching PID and remove the PCB from the WQ and return
    //        the PCB pointer. If no match is found, return invalid PID.
    // INPT : long pid
    // OUTP : N/A
    ...
    // HW2 //
    // *****
    // NAME : io_getcSystemCall()
    // DESC : Returns the start of input operation event code
    // INPT : N/A
    // OUTP : N/A
    ...
    // HW2 //
    // *****
    // NAME : io_putcSystemCall()
    // DESC : Returns the start of output operation event code
    // INPT : N/A
    // OUTP : N/A
